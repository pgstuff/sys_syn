...
CREATE TABLE user_data.parent_table (
        parent_table_key_1 integer NOT NULL,
        parent_table_key_2 integer NOT NULL,
        parent_table_text text,
        CONSTRAINT parent_table_pkey PRIMARY KEY (parent_table_key_1, parent_table_key_2));

CREATE TABLE user_data.child_table (
        child_table_key integer NOT NULL,
        parent_table_key_1 integer,
        parent_table_key_2 integer,
...
SELECT sys_syn.in_table_add (
                ...
                in_columns      => ARRAY[
                       ...
                       $COL$("parent_table_key_1","integer",Attribute,"in_source.parent_table_key_1",,1,"parent_table","parent_table_key_1")$COL$,
                       $COL$("parent_table_key_2","integer",Attribute,"in_source.parent_table_key_2",,1,"parent_table","parent_table_key_2")$COL$
                ]::sys_syn.create_in_column[],
                full_table_reference    => 'user_data.child_table'
        );

INSERT INTO user_data.parent_table(
        parent_table_key_1,     parent_table_key_2,     parent_table_text)
VALUES (1,                      2,                      'parent_data');

INSERT INTO user_data.child_table(
        child_table_key,        parent_table_key_1,     parent_table_key_2)
VALUES (3,                      1,                      2);
...
UPDATE user_data.parent_table_out_queue SET queue_state = 'Reading'::sys_syn.queue_state, hold_reason_text = 'Testing Hold' WHERE (key).parent_table_key_1 = 1;
...
UPDATE user_data.parent_table_out_queue SET queue_state = 'Reading'::sys_syn.queue_state WHERE (key).parent_table_key_1 = 1;
UPDATE user_data.parent_table_out_queue SET queue_state = 'Processed'::sys_syn.queue_state WHERE (key).parent_table_key_1 = 1;
SELECT user_data.parent_table_out_processed();
SELECT key, delta_type, queue_state FROM user_data.parent_table_out_queue;
